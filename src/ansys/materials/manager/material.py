...
class Material:
    ...
    def __init__(
        ...
        properties: Union[MaterialPropertyDict, Dict]
            Dictionary of material properties. Linear material properties are specified with the
            `PropertyCode` enum and either a float value or a np.ndarray of temperatures and
            property values.
        nonlinear_models: Dict[str, _BaseModel]
    ...
    def set_property(self, property_code: PropertyCode, value: model_type) -> None:
        """
        Set a linear material property.

        Either provide a single float, for properties that are treated as Isothermal, or provide
        an array of floats for properties that are to be treated as temperature dependent. For
        temperature-dependent properties provide a 2*N array where the first column specifies
        temperatures, and the second column provides the property values.
        """
        assert isinstance(
    ...
    def set_model(self, model_type: str, value: _BaseModel) -> None:
        ...
        assert isinstance(value, _BaseModel), "Nonlinear models must inherit from '_BaseModel'"
        self._nonlinear_models[model_type] = value

    def remove_model(self, model_code: str) -> None:
        """Clear a nonlinear material model."""
